...
function loadJson(file,callback) {
	var prs=new XMLHttpRequest();
	prs.overrideMimeType("application/json");
    prs.open("GET",file,true);
    prs.onreadystatechange=function(){

    if(prs.readyState===4 && prs.status=="200"){
    callback(prs.responseText);
}
    };
    prs.send();
    }
